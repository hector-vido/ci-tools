...
func TestCheckRepos(t *testing.T) {
	client := fakeAutomationClient{
		...
		reposWithAppInstalled: sets.New[string]("org-1/repo-a", "org-1/repo-c", "org-2/repo-z", "org-5/repo-a", "org-5/repo-b", "org-6/repo-a"),
		permissionsByRepo: map[string]map[string][]string{
			"org-1/repo-a": {
				"a-bot":                      []string{"write"},
				"b-bot":                      []string{"write"},
	...
	testCases := []struct {
		...
		{
			name:        "private repository has branch protection enabled and is not a free plan",
			repos:       []string{"org-5/repo-d"},
			mode:        standard,
			expectedErr: errors.New("branch protection is enabled, the org org-5 has a free plan, the repository repo-d must be public"),
		},
	}
